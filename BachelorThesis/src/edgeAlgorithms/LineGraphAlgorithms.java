...
/**
 * Describes a set of algorithms that work on the line graph of a graph to color
 * the nodes instead of the edges and later reverse this to get a valid edge
 * coloring.
 * 
...
public abstract class LineGraphAlgorithms implements ColoringAlgorithms {
	...
	/**
	 * Constructor for usage without the MVC pattern when no visualization is
	 * necessary
	 */
	...
	/**
	 * Constructor for usage with the MVC pattern when visualization is required
	 * 
	...
	/**
	 * Delivers the steps that were taken during the algorithm for the undone
	 * button
	 * 
	 * @return
	 */
	...
	/**
	 * Returns the original graph of a line graph.
	 * 
	 * @return
	 */
	...
	/**
	 * Applies the coloring determined by the Line Graph to the original graph
	 */
	private void colorOriginal() {
		...
		if (!original.isGraphColoringValid())
			throw new InvalidColorException("Conversion from Line Graph to Original graph failed at coloring");
	}
}
